...
import { GetAllFoldersResponse, CreateFolderRequest, CreateFolderResponse } from './dto/FolderDTO';
...
export interface CreateFolderCallbacks {
  onSuccess: (response: CreateFolderResponse) => void;
  onError: (errorCode: string, errorMessage: string) => void;
  onLoginRequired?: () => void;
  onSubscriptionRequired?: () => void;
}
...
export class FolderService {
  ...
  static async createFolder(
    ...
    callbacks: CreateFolderCallbacks,
    abortSignal?: AbortSignal
  ): Promise<void> {
    const url = `${ENV.API_BASE_URL}${this.ENDPOINT}`;
    const authHeaders = await ApiHeaders.getAuthHeaders('FolderService');
      ...
      if (response.status === 401) {
        ...
        if (TokenRefreshRetry.shouldRetryWithTokenRefresh(response, errorData)) {
          try {
            ...
            const data: CreateFolderResponse = await retryResponse.json();
            callbacks.onSuccess(data);
            return;
          } catch (refreshError) {
            console.error('[FolderService] Token refresh failed:', refreshError);
